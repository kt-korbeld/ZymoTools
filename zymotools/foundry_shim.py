...
Both workarounds are specific to a machine and a foundry version, so each can be
switched off independently with ``FOUNDRY_SHIM_PATCHES`` -- a comma-separated
subset of ``residue_cache,cuequivariance`` (empty disables both, unset applies
both). The whole shim can be bypassed a level up, with ``rfd3-job
--foundry-shim off`` or ``BINDER_PIPELINE_FOUNDRY_SHIM=off``, which runs the
foundry console scripts directly; see ``rfd3_slurm.foundry_cmd``.
...
foundry's own interpreter without this package being installed there::
    ...
    <foundry-venv>/bin/python .../zymotools/foundry_shim.py rfd3 design inputs=...
"""
...
PATCHES_ENV = "FOUNDRY_SHIM_PATCHES"
ALL_PATCHES = ("residue_cache", "cuequivariance")


def enabled_patches():
    """
    The set of workarounds to apply, from ``FOUNDRY_SHIM_PATCHES``. Unset means
    all of them; an empty value means none, which leaves the shim a pass-through.
    """
    raw = os.environ.get(PATCHES_ENV)
    if raw is None:
        return set(ALL_PATCHES)
