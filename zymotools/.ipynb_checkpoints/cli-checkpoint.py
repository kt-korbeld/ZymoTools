...
def find_wrapper_executable():
    ...
    wrapper_path = repo_root / "pipeline.py"
    # if pipeline.py is excecuted, this file should exist at repo root
    if wrapper_path.is_file():
        return [sys.executable, str(wrapper_path)]
    # if installed through pip as binder_pipeline cli tool, use that
    return [sys.executable, "-m", "binder_pipeline"]
...
def cmd_view_hotspots(args):
    ...
    """
    return launch_shiny("binder_pipeline.apps.hotspot_viewer:app", args)

def cmd_view_linker(args):
    """
    use launch_shiny to launch linker distance app
    """
    return launch_shiny("binder_pipeline.apps.linker_viewer:app", args)
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="binder-pipeline",
        description="BindCraft hotspot-screening pipeline.")
